...
describe("LLM catalog", () => {
  test("reviews pinned static snapshot gate", () => {
    ...
    expect(NINE_ROUTER_LLM_CATALOG.sourceCommit).toBe("79918c7830695bbca4a45c9fea4a42c3e9fd73d1");
    expect(NINE_ROUTER_LLM_CATALOG.sourceFiles).toEqual([...NINE_ROUTER_LLM_CATALOG.sourceFiles].sort());
    expect(new Set(NINE_ROUTER_LLM_CATALOG.sourceFiles).size).toBe(NINE_ROUTER_LLM_CATALOG.sourceFiles.length);
    expect(NINE_ROUTER_LLM_CATALOG.sourceFiles).toEqual(expect.arrayContaining([
    ...
    expect(Object.values(NINE_ROUTER_LLM_CATALOG.providers).flatMap((provider) => provider.models)).toHaveLength(468);
    expect(Object.values(NINE_ROUTER_LLM_CATALOG.providers).flatMap((provider) => provider.models).every((model) => model.kind === "llm")).toBeTrue();
    expect(Object.entries(NINE_ROUTER_LLM_CATALOG.providers).filter(([, provider]) => provider.modelsFetcher || provider.passthroughModels).map(([id]) => id)).toEqual(["grok-web", "kilocode", "kimchi", "mimo-free", "opencode", "openrouter", "perplexity-agent", "venice", "vercel-ai-gateway"]);
    expect(NINE_ROUTER_LLM_CATALOG.providers["mimo-free"].catalogKey).toBe("mmf");
    expect(NINE_ROUTER_LLM_CATALOG.providers.mmf.catalogKey).toBe("mmf");
    const routeKeyDeclarers = Object.entries(NINE_ROUTER_LLM_CATALOG.providers).flatMap(([providerId, provider]) => [provider.catalogKey, ...provider.aliases].map((key) => [key, providerId] as const)).reduce((groups, [key, providerId]) => {
      (groups[key] ??= []).push(providerId);
      return groups;
    }, {} as Record<string, string[]>);
    const collisions = Object.fromEntries(Object.entries(routeKeyDeclarers).filter(([, providerIds]) => providerIds.length > 1));
    ...
    expect(sol).toMatchObject({ providerId: "codex", modelId: "gpt-5.6-sol", model: { id: "gpt-5.6-sol", name: "GPT 5.6 Sol", reasoning: { reasoning: true, thinkingFormat: "openai" } } });
    expect(matchLlmCatalogRoute("cx/gpt-5.6-sol-review", NINE_ROUTER_LLM_CATALOG)?.model.upstreamModelId).toBe("gpt-5.6-sol");
    expect(matchLlmCatalogRoute("cx/gpt-5.6-sol-review", NINE_ROUTER_LLM_CATALOG)?.model.reasoning).toMatchObject({ reasoning: true, thinkingFormat: "openai" });
    expect(NINE_ROUTER_LLM_CATALOG.reasoningPicker.patternLevels.find((rule) => rule.pattern === "*gpt-5.6-sol*")?.levels).toEqual(["none", "minimal", "low", "medium", "high", "xhigh", "max"]);
    expect(matchLlmCatalogRoute("gcli/grok-4.5-high", NINE_ROUTER_LLM_CATALOG)?.model.reasoning).toMatchObject({ reasoning: true, thinkingFormat: "openai" });
    expect(matchLlmCatalogRoute("glm/glm-5.2", NINE_ROUTER_LLM_CATALOG)?.model.reasoning).toMatchObject({ reasoning: true, thinkingFormat: "zai", thinkingRange: null });
    expect(NINE_ROUTER_LLM_CATALOG.reasoningPicker.formatLevels.zai).toEqual(["none", "thinking"]);
    const rendered = renderCatalogModule({ catalog: NINE_ROUTER_LLM_CATALOG } as Parameters<typeof renderCatalogModule>[0]);
